...
"""Default configuration values for the testing framework."""

from importlib.resources import files
from pathlib import Path

# Project root is three levels up from this file in a source checkout:
# src/aws_durable_execution_sdk_testing/config.py -> project root
_PROJECT_ROOT: Path = Path(__file__).resolve().parent.parent.parent
_REPOSITORY_TESTS_DIR: Path = _PROJECT_ROOT / "test-requirements"
_PACKAGE_TESTS_DIR: Path = Path(str(files("aws_durable_execution_sdk_testing").joinpath("test_requirements")))
...
STACK_NAME_PREFIX = "sdk-testing-harness"
